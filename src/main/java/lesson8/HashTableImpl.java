...
public class HashTableImpl<K, V> implements HashTable<K, V> {
    ...
    public boolean put(K key, V value) {
        ...
        while (data[indexFromHashFunc] != null && data[indexFromHashFunc] != emptyItem) {
            if (isKeysEquals(data[indexFromHashFunc], key)) {//добавление листа зо знасениями соответствующему данному ключу
                List<V> sumValue = new ArrayList<>();
                sumValue.add(value);
                sumValue.add(data[indexFromHashFunc].getValue());
                data[indexFromHashFunc].setValue((V) sumValue);
                return true;
            }
